...
func patchOrCreateDockerConfigFile() error {
	...
	if err != nil {
		return fmt.Errorf("unable to retrieve user home dir to verify default container authentication: %w", err)
	}
...
`
	...
	if err != nil {
		return fmt.Errorf("unable to create .docker/config.json file: %w", err)
	}
	return nil
}
